...
TEST(Matchers, Approximately) {
  ...
  EXPECT_THAT(
      GetExplanation(Approximately(EqualsProto(R"pb(val: 0.9)pb"), 0.01), msg),
...
TEST(Matchers, TreatingNaNsAsEqual) {
  ...
  EXPECT_THAT(GetExplanation(EqualsProto(R"pb(val: nan)pb"), msg),
              HasSubstr("val: nan -> nan"));
}

TEST(Matchers, IgnoringFields) {
  const TestMessage msg = ParseTextProtoOrDie(R"pb(name: "name" num: 42)pb");
  EXPECT_THAT(msg, IgnoringFields({"mbo.testing.proto.TestMessage.num"},
                                EqualsProto(R"pb(name: "name" num: 25)pb")));
  EXPECT_THAT(msg, IgnoringFields({"mbo.testing.proto.TestMessage.name"},
                                EqualsProto("num: 42")));
}
...
TEST(Matchers, IgnoringFieldPathsNested) {
  ...
  EXPECT_THAT(
      msg, IgnoringFieldPaths({"one.name"},
                              EqualsProto(R"pb(one { num: 42 name: "different" })pb")));
  EXPECT_THAT(
      msg, Not(IgnoringFieldPaths({"one.name"},
                                  EqualsProto(R"pb(one { num: 25 name: "different" })pb"))));
}

TEST(Matchers, IgnoringFieldPathsRepeatedIndex) {
  const TestMessage2 msg = ParseTextProtoOrDie(R"pb(more {num: 10} more {num: 20})pb");
  EXPECT_THAT(
      msg, IgnoringFieldPaths({"more[0].num"},
...
TEST(Matchers, IgnoringFieldPathsRepeatedNested) {
  const TestMessage2 msg = ParseTextProtoOrDie(R"pb(more {num: 10} more {num: 20})pb");
  EXPECT_THAT(
      msg, IgnoringFieldPaths({"more.num"},
...
TEST(Matchers, IgnoringFieldPathsTerminalIndex) {
  ...
  EXPECT_DEATH(Matches(IgnoringFieldPaths({"num[0]"}, EqualsProto(R"pb(num: 1 num: 2)pb")))(msg),
...
TEST(Matchers, IgnoringRepeatedFieldOrderingNested) {
  const TestMessage2 msg = ParseTextProtoOrDie(R"pb(more {num: 10} more {num: 20})pb");
  EXPECT_THAT(msg, IgnoringRepeatedFieldOrdering(
                       EqualsProto(R"pb(more {num: 20} more {num: 10})pb")));
}
